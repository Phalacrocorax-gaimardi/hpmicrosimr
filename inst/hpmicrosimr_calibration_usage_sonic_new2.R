...
calibration <- result$History %>% as_tibble() %>% mutate(error=-Value) %>% arrange(-error) %>% select(-Value)

write_csv(calibration, "C:/Users/Joe/pkgs/hpmicrosimr/inst/macro_calibration_rBayesianOptimization.csv")
